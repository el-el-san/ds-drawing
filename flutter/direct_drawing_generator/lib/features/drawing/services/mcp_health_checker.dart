...
class McpHealthChecker {
  ...
  Future<Map<String, dynamic>> _send(
    ...
    http.Response response = await _post(body, mode: mode).timeout(timeout);
    _captureSessionFromHeaders(response, logs);
    ...
    if (!_isSuccessStatus(response.statusCode)) {
      ...
      if (_looksLikeContentTypeError(response, snippet)) {
        ...
        logs.add(
          nextMode == _ContentTypeMode.jsonWithCharset
              ? 'Content-Typeエラーを検知。charset付きで再試行します'
              : 'Content-Typeエラーを検知。application/jsonを明示して再試行します',
        );
        mode = nextMode;
        response = await _post(body, mode: mode).timeout(timeout);
        _captureSessionFromHeaders(response, logs);
        if (!_isSuccessStatus(response.statusCode)) {
